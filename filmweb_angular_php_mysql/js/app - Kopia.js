...
app.controller( 'myController', ['$scope', '$filter', 'orderByFilter', '$http', '$routeParams', '$log', function($scope, $filter, orderByFilter, $http, $routeParams, $log) {
    $scope.current = {};
    $http({ 
        url: './json.php', 
        method: 'GET', 
...
//        });
    ...
    $scope.addMovies = function() {
        ...
        method: 'POST', 
        dataType: 'json', 
        params: { action: 'add', id: $routeParams.id } 
    }).then(function(data) {
        $log.info(data.data);
        $scope.movies = data.data.posts;
        $scope.current = { id: 0, mov: $scope.movies[0]};
